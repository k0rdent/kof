...
| kcm<br>.kof<br>.operator<br>.otlp<br>.endpoint | string | `""` | OTLP endpoint override for the operator's OTel SDK. When empty, defaults to http://$(NODE_IP):4317. |
...
| victoria-metrics-multilevel-select<br>.allowPartialResponse | bool | `true` | Allow returning partial results when some of the storage nodes are down or unreachable. |
| victoria-metrics-multilevel-select<br>.enabled | bool | `true` | Enables the VictoriaMetrics metrics aggregation (multilevel-select) VMCluster. This is a select-only VMCluster (no vminsert/vmstorage) that fans queries out to the local mothership VictoriaMetrics and every regional cluster's vmselect via `VMStorageConnection` CRs, providing a single Prometheus-compatible query endpoint across all regions (replaces promxy). |
| victoria-metrics-multilevel-select<br>.extraArgs | object | `{}` |  |
| victoria-metrics-multilevel-select<br>.image<br>.tag | string | `"v1.105.0-cluster"` |  |
| victoria-metrics-multilevel-select<br>.port | string | `"8481"` |  |
| victoria-metrics-multilevel-select<br>.replicaCount | int | `3` |  |
| victoria-metrics-multilevel-select<br>.resources<br>.limits<br>.cpu | string | `"2000m"` |  |
| victoria-metrics-multilevel-select<br>.resources<br>.limits<br>.memory | string | `"2Gi"` |  |
| victoria-metrics-multilevel-select<br>.resources<br>.requests<br>.cpu | string | `"200m"` |  |
| victoria-metrics-multilevel-select<br>.resources<br>.requests<br>.memory | string | `"512Mi"` |  |
...
| victoriametrics<br>.vmalert<br>.enabled | bool | `true` | Enables the `kof-mothership-alert-rules-executor` VMAlert, which evaluates both alert and record rules against the metrics multilevel-select VMCluster (replaces promxy's rule evaluation role) and also runs VMAlertManager. |
...
| victoriametrics<br>.vmalert<br>.spec | object | `{"datasource":{"url":"http://vmselect-cluster:8481/select/0/prometheus"},`<br>`"evaluationInterval":"15s",`<br>`"extraArgs":{"http.pathPrefix":"/",`<br>`"remoteWrite.disablePathAppend":"true"},`<br>`"image":{"tag":"v1.105.0"},`<br>`"notifiers":[{"url":"http://vmalertmanager-cluster:9093"}],`<br>`"port":"8080",`<br>`"remoteRead":{"url":"http://vmselect-cluster:8481/select/0/prometheus"},`<br>`"remoteWrite":{"url":"http://vminsert-cluster:8480/insert/0/prometheus/api/v1/write"},`<br>`"selectAllByDefault":true}` | [VMAlertSpec](https://docs.victoriametrics.com/operator/api/#vmalertspec) `datasource.url`, `remoteRead.url` and `configMaps` are set automatically by the `vmalert.yaml` template when `victoria-metrics-multilevel-select.enabled` is true. |
