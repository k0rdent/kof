...
func (c *ChildClusterRole) DiscoverRegionalClusterConfigMapByLocation(
	...
	requirement, err := labels.NewRequirement(KofRegionalClusterConfigMapLabel, selection.Exists, nil)
	if err != nil {
	...
	for _, regionalClusterConfigmap := range regionalClusterConfigMapList.Items {
		...
		if childCloud != regionalConfigData.RegionalClusterCloud {
			log.Info("Child cloud doesn't match regional", "childCloud", childCloud, "regionalClusterCloud", regionalConfigData.RegionalClusterCloud)
			continue
		}
