...
|-----|------|---------|-------------|
| cert-manager-service-template | object | `{"helm":{"charts":[{"name":"cert-manager",`<br>`"version":"v1.16.4"}],`<br>`"repository":{"name":"cert-manager",`<br>`"url":"https://charts.jetstack.io"}},`<br>`"namespace":"kcm-system"}` | Config of `ServiceTemplate` to use `cert-manager` in `MultiClusterService`. |
