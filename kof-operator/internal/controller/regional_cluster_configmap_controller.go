...
func (r *RegionalClusterConfigMapReconciler) Reconcile(
	...
	if errors.IsNotFound(err) {
		return ResourceCleanup(ctx, r.Client, req.Name)
	}
...
func ResourceCleanup(ctx context.Context, client client.Client, cmName string) (ctrl.Result, error) {
	if err := vmuser.NewManager(client).Delete(ctx, cmName); err != nil {
