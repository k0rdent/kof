...
| grafana<br>.dashboard<br>.datasource | object | `{"regex":"/promxy/"}` | Datasource configuration |
| grafana<br>.dashboard<br>.datasource<br>.regex | string | `"/promxy/"` | Regex pattern to filter datasources. |
| grafana<br>.dashboard<br>.filters | object | `{"clusterName":"mothership"}` | Filters configuration |
| grafana<br>.dashboard<br>.filters<br>.clusterName | string | `"mothership"` | Filters data by cluster name. |
