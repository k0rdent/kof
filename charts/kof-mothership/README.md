...
| file://../kof-file-watcher/ | kof-file-watcher | 1.11.0-rc0 |
...
| kof-file-watcher | object | `{"enabled":true,`<br>`"nodeSelector":{"node-role.kubernetes.io/control-plane":""},`<br>`"replicas":1,`<br>`"tolerations":[{"effect":"NoSchedule",`<br>`"key":"node-role.kubernetes.io/control-plane",`<br>`"operator":"Exists"}]}` | File watcher Deployment that watches a configurable filesystem path on the control-plane and emits logs and Prometheus metrics on changes or deletions. Values are passed directly to the kof-file-watcher sub-chart. |
