...
| kof-file-watcher | object | `{"baseline":{"enabled":true},`<br>`"enabled":true,`<br>`"replicas":1,`<br>`"serviceMonitor":{"enabled":true},`<br>`"watchPath":"/etc/kubernetes/audit-policy.yaml"}` | File watcher Deployment that watches a configurable filesystem path on the control-plane and emits logs and Prometheus metrics on changes or deletions. Values are passed directly to the kof-file-watcher sub-chart. |
